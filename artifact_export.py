...
def _write_readme(
    ...
    # Embed any charts generated by the render stage
    chart_lines = _chart_section(gap_dir)
    if chart_lines:
        lines += ["", "---", ""] + chart_lines
...
def _chart_section(gap_dir: Path) -> List[str]:
    """Return markdown lines embedding any charts found in documents/ subdirs."""
    lines: List[str] = []
    docs_dir = gap_dir / "documents"
    ...
    for src_dir in sorted(docs_dir.iterdir()):
        if not src_dir.is_dir():
            continue
        charts = sorted(src_dir.glob("chart_*.png"))
        for chart in charts:
            rel = f"documents/{src_dir.name}/{chart.name}"
            caption = chart.stem.replace("chart_", "").replace("_", " ").title()
            lines.append(f"## Data Chart: {caption}")
            lines.append("")
